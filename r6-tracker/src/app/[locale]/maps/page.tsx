  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-7xl mx-auto">
        <SectionHeader title={t('title')} description={t('description')} icon="pi-map" useLogo={true} />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-white/80 mb-2">{t('filters.search')}</label>
              <div className="relative">
                <i className="pi pi-search absolute left-3 top-1/2 transform -translate-y-1/2 text-white/50"></i>
                <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-white/5 border border-white/20 rounded-xl text-white" placeholder={t('filters.searchPlaceholder')} />
              </div>
            </div>
            <div>
              <label htmlFor="map-playlist-select" className="block text-sm font-medium text-white/80 mb-2">{t('filters.playlist')}</label>
              <select 
