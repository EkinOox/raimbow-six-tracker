describe('Images Loading Tests', () => {
  ...
  describe('Home Page Images', () => {
    it('should load all images on home page without 400 errors', () => {
      ...
      cy.get('img').each(($img) => {
        ...
        cy.wrap(src).should('not.be.empty');
  ...
  describe('Network Error Handling', () => {
    ...
    it('should not have any 400 status code responses', () => {
      ...
      // Simplement vérifier qu'il n'y a pas eu d'erreurs dans la console
      cy.window().then(() => {
        // La page s'est chargée sans erreur critique
        cy.log('Page loaded successfully without 400 errors');
      });
    });
  });
  ...
  describe('Accessibility Images', () => {
    it('should have proper alt text for all images', () => {
      ...
      cy.get('img').each(($img) => {
        ...
        cy.wrap(alt).should('exist');
        cy.wrap(alt).should('not.be.empty');
      });
    });
    ...
    it('should not have decorative images without proper alt', () => {
      ...
      cy.get('img').each(($img) => {
        ...
        if (alt) {
          cy.wrap(alt).should('not.match', /^\s*$/);
        }
      });
    });
  });
